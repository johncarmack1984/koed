...
describeDb("memory repository visibility", () => {
  ...
  it("backfills Memory Event payload encryption idempotently and fails closed on provider errors", async () => {
    ...
    await pool.query(
      "delete from encrypted_field_payloads where owner_user_id = $1",
      [owner.id]
    );
  ...
  it("keeps LCM graph and retrieval working with encrypted Memory Event and Node fields present", async () => {
    const encryptedRepo = createEncryptedPayloadRepository(pool);
    const provider = createLocalTestKeyEnvelopeEncryptionProvider(
    ...
    const graphNode = await repo.getLcmGraphNode(
      { userId: alice.id },
      leafNodeId
    );
    ...
    expect(search.results[0]?.summaryText).toContain(
